...
from utils import reduce_img_size, load_images

_VGG_MEAN = [134.93130283, 108.32459652, 91.30616992]
...
class Vgg19:
    ...
    _preds = None
    "The predictions tensor, shape of [?, 73]"
    ...
    def __init__(self,
        ...
        # For predicting.
        self._preds = tf.nn.sigmoid(self._fc8, name="prediction")

        # For training.
        self._loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(self._fc8,
                                                             self._train_labels), name='loss')
    ...
    def preds(self):
        """
        The prediction(s) tensor, shape of [?, 73].
        """
        return self._preds
    ...
    def train_labels(self):
        """
        The train labels tensor, shape of [?, 73].
        """
    ...
    def _get_conv_filter(self, name):
        initial = tf.truncated_normal(shape=self.model[name][0].shape, stddev=0.1, dtype=tf.float32)
        var = tf.Variable(initial_value=initial, name="filter", dtype=tf.float32)
        tf.add_to_collection("vars", var)
        return var

    def _get_bias(self, name):
        initial = tf.constant(0.1, shape=self.model[name][1].shape, dtype=tf.float32)
        var = tf.Variable(initial_value=initial, name="biases", dtype=tf.float32)
        tf.add_to_collection("vars", var)
        return var

    def _get_fc_weight(self, name):
        var = tf.Variable(self.model[name][0], name="weights", dtype=tf.float32)
        tf.add_to_collection("vars", var)
        return var
